...
<?php
}elseif ($this->uri->segment(2) == "forms_editor" || $this->uri->segment(2) == "lihat_pengaduan" || $this->uri->segment(2) == "tinjau_pengaduan") { ?>
  <script src="<?php echo base_url(); ?>assets/modules/summernote/summernote-bs4.js"></script>
...
<?php
  ...
  <script src="<?php echo base_url(); ?>assets/modules/chart.min.js"></script>
<?php
}elseif ($this->uri->segment(2) == "laporan_pengaduan") { ?>
<!-- Datatables -->
  ...
  $('.tombol-tolak').on('click', function(e) {
    ...
    swal({
    title: 'Anda Menolak Laporan ini?',
    text: "Anda tidak akan dapat mengembalikan aksi ini!",
    icon: 'warning',
    showCancelButton: true,
    confirmButtonColor: '#6777EF',
...
if (flashDataLogin) {
  iziToast.success({
    title: "<?= 'Selamat Datang ' . $this->session->userdata('nama_depan') . '!' ?>",
